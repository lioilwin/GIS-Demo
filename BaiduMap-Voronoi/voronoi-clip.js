...
function showVoronoi(options) {
    ...
    // 计算voronoi
    var diagram = new Voronoi().compute(sites, {
        xl: xMin,
        xr: xMax,
        yt: yMin,
        yb: yMax
    ...
    map.addOverlay(new BMap.CanvasLayer({
        update: function () {
            ...
            // cells
            var iCells = diagram.cells.length;
            while (iCells--) {
                var cell = diagram.cells[iCells];
                var site = cell.site;
                ctx.fillStyle = site.fill; // 填充颜色
                ctx.beginPath();
                for (var i = 0; i < cell.halfedges.length; i++) {
                    var v = toPx(cell.halfedges[i].getStartpoint());
                    if (i == 0)
                        ctx.moveTo(v.x, v.y);
                    else
                        ctx.lineTo(v.x, v.y);
                ...
                // site
                if (site.size) {
                    ctx.fillStyle = site.color || strokeStyle; // 填充颜色                
                    ctx.beginPath();
                    var v = toPx(site);
                    ctx.rect(v.x, v.y, site.size, site.size);
                    // ctx.arc(v.x, v.y, site.size, 0, 2 * Math.PI);
                    ctx.fill();
                }
            }
            ctx.restore(); // 恢复
    ...
    // 转像素
    function toPx(v) {
        return map.pointToPixel(new BMap.Point(v.x / times, v.y / times));
    }
}